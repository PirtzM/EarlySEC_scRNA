################################################################################
#####################Analysis of Late Samples (6 total)#########################
##############################Epi Subset########################################
...
EpiOnly <- FindNeighbors(object = EpiOnly,           # Seurat object
                         ...
                         graph.name = 'LateMiceEpi.DF.Clusters')    # Name for stored SNN graph

EpiOnly <- FindClusters(object = EpiOnly,            # Seurat object 
                        resolution = 0.7,
                        graph.name = 'LateMiceEpi.DF.Clusters') # Resolution parameter (smaller the number the fewer clusters) 
